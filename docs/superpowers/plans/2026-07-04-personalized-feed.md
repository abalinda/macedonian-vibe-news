# Personalized Feed («За тебе») + Profile («Профил») Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** A signed-in-only personalized feed at `/za-tebe` (category picks + click-learning "points-jar" ranking) and a private profile page at `/profil` (Clerk account, vibe profile, reading history, saved articles).
...
- Personalized pages (`/za-tebe`, `/profil`) must use `export const dynamic = "force-dynamic";` and **no `revalidate`**. Never add `auth()`/`currentUser()` to existing ISR pages (`/`, `/najnovo`, `/blog/[id]`).
- Personalization must never break core behavior: every learn/rank call is wrapped in try/catch; `/go` always redirects; `/za-tebe` falls back to newest-first on ranking errors.
...
### Task 6: «За тебе» page — wizard, vibe profile, personalized feed, nav

**Files:**
- Create: `web/app/za-tebe/page.tsx`
- Create: `web/app/za-tebe/vibe-wizard.tsx`
...
- Modify: `web/app/najnovo/latest-feed.tsx` (accept a `feed` prop so `/za-tebe` reuses it)
...
- Produces: `VibeProfile({ shares, showChangeButton?: boolean })` React component reused by Task 8; `LatestFeed` gains optional prop `feed?: ArticleFeed` (default `"latest"` — `/najnovo` behavior unchanged); `ArticleFeed` union becomes `"home" | "latest" | "archive" | "search" | "za-tebe" | "profile"`.
...
```tsx
// web/app/za-tebe/vibe-wizard.tsx
'use client';
...
```
(The `/za-tebe` page passes `activeCategory="ForYou"`, so the chip highlights there. Existing pages pass other values — no other changes needed. Note the file has local uncommitted modifications on this branch — edit around them, do not revert them.)
...
Run (from `web/`): `npm run dev`, then:
1. Signed out → `http://localhost:3000/za-tebe` shows the pitch + working «Најава» modal; nav drawer and category chips show «За тебе».
2. Sign in → `/za-tebe` shows the wizard; «Зачувај» disabled with zero picks; pick Технологија + Спорт → save → feed renders with a «Твојот вајб» panel on top; Tech/Sports articles cluster near the top; other categories still present below (boost, don't hide).
3. `/za-tebe?izberi=1` → wizard reopens pre-filled with Tech + Sports and shows the reset warning line.
...
Run (from `web/`): `npx tsc --noEmit && npx eslint app/za-tebe app/_components/vibe-profile.tsx app/_components/article-link.tsx app/najnovo/latest-feed.tsx`

```bash
git add web/app/za-tebe web/app/_components/vibe-profile.tsx web/app/_components/article-link.tsx web/app/najnovo/latest-feed.tsx web/app/_components/navigation.tsx web/DESIGN_SYSTEM.md
git commit -m "feat(web): «За тебе» personalized feed — wizard, vibe profile, ranked feed, nav
...
In `web/app/najnovo/latest-feed.tsx` (this serves both `/najnovo` and `/za-tebe`; `/najnovo` is ISR so saved state MUST load client-side):
...
2. Signed in → all four sections render: vibe bars match `/za-tebe`; «Прочитано» shows articles you clicked in Tasks 5–7 testing; «Зачувано» lists what you saved (unsave removes a row without navigation); «Сметка» shows the styled Clerk panel and its tabs work (hash routing — no 404s).
...
1. Signed out: `/za-tebe` pitch, `/profil` prompt, `/najnovo` unchanged, `/go/<id>` redirects fast, no save buttons anywhere.
2. Sign in fresh (or wipe your rows: `DELETE FROM user_category_prefs WHERE user_id='<you>'` etc.): wizard appears on `/za-tebe`; save Технологија + Спорт.
3. DB shows 6 jar rows: picked ones weight 5, others 1.
4. Click 3 Culture articles → `user_clicks` rows exist; Culture jar ≈ 4.0; Culture stories climb on `/za-tebe` reload; re-clicking the same article within 24h does not add points.
5. Vibe bars on `/za-tebe` and `/profil` agree and roughly match jar shares.
6. «Смени ги вибрациите» → wizard pre-filled → save → jars reseeded (Culture learning gone).
7. Save + unsave articles from `/najnovo` and `/za-tebe`; state survives reload; «Зачувано» on `/profil` matches; saving bumped the jar.
