# Personalized Feed («Твои Вести») — Design Spec
...
A new page on vibes.mk, **«Твои Вести»** ("For You"), that shows signed-in users a news feed ordered by *their* interests instead of pure newest-first.
...
| Where it lives | New dedicated page `/tvoi-vesti`; homepage and Најново stay untouched |
...
| `web/app/tvoi-vesti/page.tsx` | Server component. Signed out → pitch + `SignInButton`. Signed in, no pref rows → wizard. Otherwise → profile header + personalized feed. Dynamic rendering (Clerk makes it dynamic anyway); **no ISR**. |
| `web/app/tvoi-vesti/vibe-wizard.tsx` | Client component: 6 category cards (multi-select, min 1), «Зачувај» calls `savePreferences`, then refresh. Also rendered when re-triggered via «Смени ги вибрациите» (with copy warning that saving restarts the learning). |
| `web/app/tvoi-vesti/vibe-profile.tsx` | Client component: «Твојот вајб» bars (percent shares) + «Смени ги вибрациите» button that reopens the wizard. |
| `web/app/tvoi-vesti/personal-feed.tsx` | Client list component, modeled on `najnovo/latest-feed.tsx`: initial 18 posts, «Уште» load-more (+18), uses shared `ArticleLink` with `feed="tvoi-vesti"`. |
...
| `web/app/_components/navigation.tsx` | Add «Твои Вести» to `menuLinks` and the `CategoryNav` array (visible to everyone; the page itself handles the signed-out pitch). |
| `web/app/_components/article-link.tsx` | Extend the `feed` prop union with `'tvoi-vesti'` for PostHog `article_click`. |
...
/tvoi-vesti (server) ──▶ getJars + SELECT latest 150 posts ──▶ personalization.rankPosts (pure) ──▶ rendered feed
profile bars ◀── personalization.profileShares (same jars)
```

PostHog keeps receiving `article_click` exactly as today (now also with `feed: 'tvoi-vesti'`) — analytics and the ranking algorithm are deliberately independent systems.
...
- Page title: **«Твои Вести»**; nav label «ЗА ТЕБЕ».
...
| Signed out on `/tvoi-vesti` | Pitch + sign-in; no DB reads. |
...
2. **Manual E2E checklist** (local `npm run dev` against real Turso):
   - Signed out → `/tvoi-vesti` shows pitch; nav link visible.
...
Signed-in only (signed out → same pitch-and-sign-in pattern as `/tvoi-vesti`). Private — a user only ever sees their own data. Four stacked sections, all per DESIGN_SYSTEM.md:
...
2. **Твојот вајб** — reuses the exact `vibe-profile.tsx` component from `/tvoi-vesti` (bars + «Смени ги вибрациите», which links to `/tvoi-vesti` with the wizard open).
...
- **Placement (v1):** cards on `/tvoi-vesti`, `/najnovo`, and `/profil`'s saved/history lists. Homepage and `/all` are skipped in v1 (home is a shared ISR page — per-user saved-state would break its caching; `/all` can follow later using the same component).
...
- Manual checklist additions: save/unsave from `/tvoi-vesti` and `/najnovo` (button doesn't navigate!), state survives reload and appears on `/profil`; saving bumps the jar (verify in DB); reading history shows the 3 Culture clicks from the earlier checklist step; Clerk account panel loads styled.
